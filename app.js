...
function initApp() {
  ...
  canvas.addEventListener('mousedown', (e) => {
    ...
    } else if (currentTool === 'crop' || currentTool === 'marquee') {
  ...
  canvas.addEventListener('mousemove', (e) => {
    ...
    if (currentTool === 'brush' || currentTool === 'eraser') {
      ...
      const sizeOnScreen = globalBrushSize * (currentZoom / 100);
      brushCursor.style.width = `${sizeOnScreen}px`;
      brushCursor.style.height = `${sizeOnScreen}px`;
    } else {
    ...
    if (currentTool === 'brush') {
      ...
      if (currentTool === 'crop') {
        ...
        octx.clearRect(startX, startY, x - startX, y - startY);
      }
      
      octx.strokeStyle = currentTool === 'crop' ? '#fff' : '#000';
      octx.lineWidth = 1;
    ...
    if (isDrawing) {
      if (currentTool === 'brush' || currentTool === 'eraser') {
        ...
        if (width > 10 && height > 10) {
          ...
          saveState('Crop Image');
          setTool('crop');
          showToast(`Image cropped to ${Math.round(width)}x${Math.round(height)}`);
        }
      } else if (currentTool === 'marquee') {
        // Selection tool logic can be added here
      }
    }
    isDrawing = false;
  };
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === '[') {
      globalBrushSize = Math.max(1, globalBrushSize - 5);
      document.getElementById('brushSize').value = globalBrushSize;
      document.getElementById('brushSizeLabel').textContent = globalBrushSize;
    } else if (e.key === ']') {
      globalBrushSize = Math.min(200, globalBrushSize + 5);
      document.getElementById('brushSize').value = globalBrushSize;
      document.getElementById('brushSizeLabel').textContent = globalBrushSize;
    }
  });
...
function updateBrushSize() {
  globalBrushSize = parseInt(document.getElementById('brushSize').value);
  document.getElementById('brushSizeLabel').textContent = globalBrushSize;
}
...
function handleZoomInput(val) {
  ...
  level = Math.max(10, Math.min(level, 2000));
  setZoom(level);
}
...
function setZoom(level) { 
  ...
  if (zoomInput) zoomInput.value = `${level}%`;
  const statusZoom = document.getElementById('statusZoom');
  if (statusZoom) statusZoom.textContent = `${level}%`;
  const container = document.getElementById('canvasContainer');
  if (container) container.style.transform = `scale(${level / 100})`;
}
...
function zoomIn() { 
  let current = parseInt(document.getElementById('zoomInput').value);
  if (isNaN(current)) current = 100;
  setZoom(Math.min(current + 25, 2000)); 
}
function zoomOut() { 
  let current = parseInt(document.getElementById('zoomInput').value);
  if (isNaN(current)) current = 100;
...
function applyPreset(preset) {
  resetAdjustments(); // start from baseline
  
  const presets = {
    cinematic: { exposure: -10, contrast: 25, highlights: -20, shadows: 15, temperature: -5, vibrance: 10, vignette: 30, clarity: 15 },
    vintage: { exposure: 5, contrast: -15, blacks: 30, temperature: 15, saturation: -20, grain: 40, vignette: 20 },
    golden: { temperature: 25, tint: 10, exposure: 5, highlights: 10, shadows: 15, vibrance: 25 },
    cyberpunk: { temperature: -30, tint: 25, contrast: 35, shadows: -15, vibrance: 40, clarity: 20 },
    bwdrama: { saturation: -100, contrast: 40, highlights: -30, shadows: -20, clarity: 30, vignette: 40 },
    polaroid: { contrast: -20, whites: -15, blacks: 40, temperature: 10, saturation: -30, grain: 20 },
    coldblue: { temperature: -40, tint: -10, contrast: 15, shadows: 10, saturation: -10 },
    punchy: { exposure: 5, contrast: 30, highlights: -10, shadows: -10, clarity: 25, vibrance: 20, saturation: 10 },
    y2knostalgia: { contrast: 15, highlights: 20, temperature: -5, tint: 15, vibrance: 25, clarity: -10 },
    lofidream: { exposure: 10, contrast: -20, shadows: 25, temperature: 10, grain: 30, blur: 3, clarity: -15 },
    neonglow: { contrast: 30, highlights: 15, blacks: -20, temperature: -20, tint: 20, vibrance: 50, saturation: 10 },
    cleangirl: { exposure: 15, contrast: -5, highlights: -10, shadows: 10, temperature: -5, clarity: 5, vibrance: 10 },
    moodyforest: { exposure: -15, contrast: 20, highlights: -25, shadows: 5, temperature: -10, tint: -20, vibrance: -10 },
    kodakportra: { exposure: 5, contrast: -10, blacks: 15, temperature: 12, tint: -5, vibrance: 15, grain: 15 },
    fujisuperia: { contrast: 25, shadows: -10, temperature: -8, tint: 12, vibrance: 20, grain: 20 },
    softpastel: { exposure: 20, contrast: -15, highlights: -15, shadows: 20, vibrance: 30, clarity: -5 },
    darkacademia: { exposure: -10, contrast: 25, shadows: -10, temperature: 20, tint: 5, saturation: -25 },
    cyberglitch: { contrast: 45, highlights: 30, shadows: -30, tint: 40, vibrance: 60, clarity: 30 },
    analogdisp: { exposure: 15, highlights: 25, blacks: 30, temperature: 15, grain: 50, vignette: 35 },
    minimalwhite: { exposure: 25, contrast: 5, highlights: 15, temperature: -10, saturation: -40, vibrance: -10 },
    retro90s: { contrast: 15, blacks: 20, temperature: 20, tint: -5, vibrance: -10, saturation: 20, grain: 25 },
    desertmirage: { exposure: 5, contrast: 30, highlights: 15, temperature: 40, tint: 5, vibrance: -15, vignette: 20 },
    midnightcity: { exposure: -5, contrast: 30, blacks: -25, temperature: -30, vibrance: 40, highlights: 25 },
    etherealglow: { exposure: 25, contrast: -20, shadows: 30, vibrance: 15, clarity: -25, blur: 5 },
    sunsetblvd: { exposure: 5, contrast: 10, temperature: 35, tint: 25, shadows: 15, vibrance: 30 },
    urbangrime: { contrast: 40, highlights: -20, shadows: 20, temperature: -5, tint: -15, saturation: -30, clarity: 40 },
    softgrunge: { exposure: 5, contrast: 10, blacks: 40, temperature: -15, saturation: -15, grain: 45 },
    vibranthdr: { shadows: 40, highlights: -40, clarity: 50, vibrance: 50, saturation: 15 }
  };
